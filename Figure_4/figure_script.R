...
dtB <- tibble(text = c(bquote(italic(Assister-Distorter)),
                      ...
                      x =  c(200,430,430), y = c(-0.15, 0.0820 + delta, -0.0284-delta),
...
yvals <- c(0.9, 0.65, 0.47, 0.20, 0.0975, 0.05, 0.4, 0.17)
xvals <- c(rep(300, 4), 460, 460, 35, 35)
angle <- c(rep(0, 6), 1, 1)
...
pC <- dC4 %>% filter(Hatch == 1) %>% ggplot(aes(G, Frequency, fill = Haplotype)) + 
  ...
  geom_rect(aes(xmin = 420, xmax = 500, ymin = 0.035, ymax = 0.065), fill = "white") +
  geom_rect(aes(xmin = 420, xmax = 500, ymin = 0.085, ymax = 0.11), fill = "white") +

  
  annotate('rect', xmin = 420, xmax = 500, ymin = 0.035, ymax = 0.065, 
            alpha = 0.8, fill = vircol[12]) +
  annotate('rect', xmin = 420, xmax =500, ymin = 0.085, ymax = 0.11, 
...
# pC <- dC4 %>% filter(Hatch == 1) %>% ggplot(aes(G, Frequency, fill = Haplotype)) + 
#   geom_area_pattern(aes(pattern = Hatch), 
#                     color = 'black', alpha = 0.8, pattern = "stripe") +
#   geom_area(dC4 %>% filter(Hatch == 0), aes(G, Frequency, fill = Haplotype)),
#             color = "black", alpha = 0.8) +
#   theme(plot.background = element_blank(),
#         panel.background = element_blank()) + 
#   scale_fill_viridis(discrete = T) +
#   geom_text(data = dtC, aes(x = xvals, y = yvals, label = text, angle = 90*angle), 
#             inherit.aes = F, parse = T) +
#   scale_x_continuous(limits = c(0, 1001), expand = c(0,0)) +
#   scale_y_continuous(limits = c(0, 1.001), expand =c(0.0, 0),
#                      breaks = c(0, 0.25, 0.5, 0.75, 1)) + 
#   labs(x = "Generations", y = "Frequency") +
#   theme(panel.background = element_rect(fill = "white", color = "black"),
...
# NOTE: There is something with k = 1, s = 1 being assigned [D2] = 1 that messes
# with the plot. I decided to replace this [D2] with the value from the 
# neighbouring simulation (k = 0.99, s = 1), which should hold approximately.
# Any reason why we don't simulate this EXACTLY?
...
pdf(file = "2024_12_30_Figure_4.pdf", width = 10, height = 10)
plot
dev.off()

png(file = "2024_12_30_Figure_4.png", width = 10, height = 10, units = "in", res = 1000)
plot
dev.off()
