...
for (a in 1:n){
  ...
  for (b in 1:n){
    kD <- b/(2*n)
    DF<- SAfunction(1000,kD,s,t,0.5,0.01,0.001,0.001)[[1]]
    SR <- SAfunction(1000,kD,s,t,0.5,0.01,0.001,0.001)[[2]]
    kDvector[i]<-kD
    svector[i]<-s
    DFvector[i]<-DF
    SRvector[i]<-SR
...
write.table(x = d, file = "data_4D.txt", row.names = F, col.names = T)

d <- tibble(k = kDvector, s = svector, SR = SR)
write.table(x = d, file = "data_4E.txt", row.names = F, col.names = T)