...
dtA <- tibble(text = c('X', 'Y',
              ...
              x = c(450, 350, 450, 450), y = c(0.434 - delta, 0.566 + delta, 
                                               0.566 + delta, 0.138 + delta),
...
dtB <- tibble(text = c(bquote(italic(Assister-Distorter)),
              ...
              x = c(390,430,430), y = c(0.191 + delta, 0.064 + delta, 0.008 + delta),
...
yvals <- c(0.9, 0.7, 0.5, 0.23, 0.13, 0.06)
xvals <- c(rep(250, 4), 350, 250)
...
pC <- dC4 %>% filter(Hatch == 1, G <= 500) %>% ggplot(aes(G, Frequency, fill = Haplotype)) + 
  ...
  geom_rect(aes(xmin = 310, xmax = 390, ymin = 0.115, ymax = 0.145), fill = "white") +
  
  
  annotate('rect', xmin = 210, xmax = 290, ymin = 0.045, ymax = 0.075, 
           alpha = 0.8, fill = vircol[8]) +
  annotate('rect', xmin = 310, xmax = 390, ymin = 0.115, ymax = 0.145, 
...
pdf(file = "2024_12_30_Figure_2.pdf", width = 10, height = 10)
plot
dev.off()

png(file = "2024_12_30_Figure_2.png", width = 10, height = 10, units = "in", res = 1000)
plot
dev.off()
